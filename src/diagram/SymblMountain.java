 ...
 * @author akitaya
 */
public class SymblMountain implements DiagramSymbol {

    int begin[], end[];
    OriVertex sv, ev;
    DiagramVertex sv1, ev1;
    static Stroke stroke = new BasicStroke(2, BasicStroke.CAP_ROUND,
            BasicStroke.JOIN_BEVEL, 0, new float[]{15, 5, 1, 5, 1, 5}, 0);
    static Color color = new Color(100, 0, 0);
    final static String svgStart = " <line style=\"stroke:#640000; stroke-width:4; stroke-dasharray:20, 5, 2, 5, 2, 5;\" ";

    public SymblMountain(int beginX, int beginY, int endX, int endY) {
        begin = new int[2];
        end = new int[2];
        begin[0] = beginX;
        begin[1] = beginY;
    ...
    public SymblMountain(OriVertex sv, OriVertex ev) {
        this.sv = sv;
        this.ev = ev;
    }
    public SymblMountain(DiagramVertex sv, DiagramVertex ev) {
        this.sv1 = sv;
        this.ev1 = ev;
    }
    ...
    public void paint(Graphics2D g2d, boolean isFlipped) {
        if (isFlipped) {
            g2d.setColor(SymblValley.color);
            g2d.setStroke(SymblValley.stroke);
        } else {
            g2d.setColor(color);
            g2d.setStroke(stroke);
        }
    ...
    public String toSVG(double scale, double localScale, double centerX, double centerY, boolean isFlipped) {
        String svg = (isFlipped ? SymblValley.svgStart : svgStart) + "x1=\"";
